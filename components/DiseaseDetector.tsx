...
const DiseaseDetector: React.FC = () => {
  ...
  // Stop speaking when switching results or leaving
  useEffect(() => {
    return () => VoiceService.getInstance().stop();
  }, []);
  ...
  const toggleVoice = () => {
    if (isSpeaking) {
      ...
      const treatment = language === 'en' 
        ? `Treatment steps include: ${result.treatment.join(', ')}` 
        : `Los pasos del tratamiento incluyen: ${result.regionalTreatment?.join(', ')}`;
      
      const fullText = `${name}. ${desc}. ${treatment}`;
      
      setIsSpeaking(true);
      VoiceService.getInstance().speak(fullText, language, () => setIsSpeaking(false));
    }
  };
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          >
            {selectedImage ? (
              ...
              >
                ...
                <p className="text-slate-400 mt-2 text-sm max-w-[200px] mx-auto">Or click to browse database</p>
              </div>
            )}
          ...
          <div className="bg-white p-4 rounded-2xl border border-slate-200 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center ${userLocation ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-100 text-slate-400'}`}>
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                </svg>
              </div>
              <span className="text-xs font-bold text-slate-900">Geo-Tag: {userLocation ? 'Active' : 'Pending'}</span>
            </div>
          ...
          <button
            ...
            className={`
              ...
              ${!selectedImage || isDetecting ? 'bg-slate-300 shadow-none' : 'bg-emerald-600 hover:bg-emerald-700 shadow-emerald-600/30'}
            `}
          >
            {isDetecting ? "Analyzing..." : "Launch Analysis"}
          </button>
        </div>
          ...
          <div className="bg-white rounded-[2rem] border border-slate-200 shadow-xl p-8 lg:p-10 h-full flex flex-col relative overflow-hidden">
            {isDetecting && (
              <div className="absolute inset-0 z-10 bg-white/80 backdrop-blur-sm flex flex-col items-center justify-center space-y-4">
                ...
                <p className="font-bold text-slate-600">Cross-referencing agricultural database...</p>
              </div>
            )}
            ...
            {result && !isDetecting && (
              <div className="space-y-8 animate-slideUp flex-1 flex flex-col">
                ...
                <div className="flex-1 space-y-4">
                  <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest">Treatment Protocol</h4>
